...
?>
 ...
 <section class="tabla">
                ...
                <td bgcolor="#CCCCCC"  ><table width="1056" class="tablesorter" id="myTable">
                  <thead>
                    <tr>
					  ...
					  <th width="7%" bgcolor="#313630" class="style11"><div align="center">PMB</div></th>
                      ...
                      <th width="3%" bgcolor="#313630"><div align="center"></div></th>
                    </tr>
                  </thead>
                  <tbody>
                    <?
	...
	$consulta="select recepcion.nombre, recepcion.fromm, date_format(recepcion.fecha_recepcion, '%m/%d/%Y'), tipo_entradas.nombre, fleteras.nombre, recepcion.peso, recepcion.traking, recepcion.pmb, recepcion.costo, usuarios.nombre, recepcion.id, TIMESTAMPDIFF(MONTH, fecha_recepcion, now()) as meses, TIMESTAMPDIFF(DAY,DATE_ADD(fecha_recepcion,INTERVAL TIMESTAMPDIFF(MONTH, fecha_recepcion, now()) MONTH), now()) as dias, recepcion.tipo, recepcion.cod, planes.tabla_pallets, entrada from recepcion inner join tipo_entradas on recepcion.tipo=tipo_entradas.id inner join fleteras on fleteras.id=recepcion.fletera inner join ubicacion on ubicacion.id=recepcion.ubicacion inner join usuarios on usuarios.id=recepcion.id_empleado_recibe inner join clientes on clientes.pmb=recepcion.pmb inner join planes on clientes.id_plan=planes.id where recepcion.pmb=$pmb and MONTH(fecha_recepcion)= $mes and YEAR(fecha_recepcion)=$anio order by recepcion.entrada";
//echo"$consulta";
