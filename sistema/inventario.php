...
include "menu_f.php";    }?>
 ...
 <section class="tabla">
                ...
                <td bgcolor="#CCCCCC"  ><table width="1056" class="tablesorter" id="myTable">
                  <thead>
                    <tr>
                      ...
                      <th width="3%" bgcolor="#313630"><div align="center"><span class="style11">Ubicacion</span></div></th>
                    </tr>
                  </thead>
                  <tbody>
                    <?
	...
	$consulta="select recepcion.nombre, recepcion.fromm, date_format(recepcion.fecha_recepcion, '%m/%d/%Y'), tipo_entradas.nombre, fleteras.nombre, recepcion.peso, recepcion.traking, recepcion.pmb, recepcion.costo, usuarios.nombre, recepcion.id, TIMESTAMPDIFF(MONTH, fecha_recepcion, now()) as meses, TIMESTAMPDIFF(DAY,DATE_ADD(fecha_recepcion,INTERVAL TIMESTAMPDIFF(MONTH, fecha_recepcion, now()) MONTH), now()) as dias, recepcion.tipo, recepcion.cod, planes.tabla_pallets, entrada, ubicacion.nombre from recepcion inner join tipo_entradas on recepcion.tipo=tipo_entradas.id inner join fleteras on fleteras.id=recepcion.fletera inner join ubicacion on ubicacion.id=recepcion.ubicacion inner join usuarios on usuarios.id=recepcion.id_empleado_recibe inner join clientes on clientes.pmb=recepcion.pmb inner join planes on clientes.id_plan=planes.id where fecha_entrega is null order by recepcion.pmb, ubicacion.nombre";
//echo"$consulta";
