...
function prepararExpress(id){
  console.log(id);
  var express = $.ajax({
    url: '/api/preparar_express.php?id='+id,
    type: 'GET',
    dataType: 'text'
  });
  ...
  express.done(function(data){
    ...
    if(data === "true"){
      ...
      alert("Hubo un error al marcar la entrega.")
    }
    });
...
function clockIn(user_id){
  var clock = $.ajax({
    url: '/api/clock_in.php?id='+user_id,
    type: 'GET',
    dataType: 'text'
  });

  clock.done(function(data){
    console.log(data);
    if(data === "true"){
      setTimeout(reload, 1000);
    } else {
      alert("Hubo un error al hacer clock in.")
    }
    });

  clock.fail(function(jqXHR, textStatus, errorThrown){
    console.log(errorThrown);
    $('#error-express').append('<p class="error">ERROR '+errorThrown+'</p>')
  });
}
...
function clockOut(user_id){
  var clock = $.ajax({
    url: '/api/clock_out.php?id='+user_id,
    type: 'GET',
    dataType: 'text'
  });
  ...
  clock.done(function(data){
    ...
    if(data === "true"){
      ...
      alert("Hubo un error al hacer clock out.")
    }
    });
